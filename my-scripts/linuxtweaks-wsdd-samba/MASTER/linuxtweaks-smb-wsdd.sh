...
# Create the systemd service for wsdd
cat <<EOF | sudo tee /etc/systemd/system/wsdd.service > /dev/null
[Unit]
...
[Service]
Type=simple
ExecStart=/bin/bash -c 'interface=$(ip -o link show | awk -F": " '\''/wlp|wlo|wlx/ && /UP/ && !/NO-CARRIER/ {print $2; exit} /eth|eno/ && /UP/ && !/NO-CARRIER/ {print $2; exit}'\'' ); echo "Found interface: $interface"; [ -n "$interface" ] && exec /usr/bin/wsdd --interface "$interface" || { echo "No suitable interface found"; exit 1; }'
User=wsdd
Group=wsdd
...
systemctl enable wsdd-sleep.service
systemctl start wsdd.service
systemctl start wsdd-sleep.service
