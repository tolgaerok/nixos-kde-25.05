...
first_run_setup() {
  ...
  # Add Flathub system-wide if missing
  flatpak remote-add --if-not-exists --system flathub https://flathub.org/repo/flathub.flatpakrepo

  # Apply system-level GTK override
  # sudo flatpak override --system --filesystem=xdg-config/gtk-4.0:ro
  # sudo flatpak override --system --unset-env=QT_QPA_PLATFORMTHEME
"'

  # Helper: get latest branch of runtime (system scope only)
  get_latest_runtime_branch() {
    local base="$1"
    local latest

    latest=$(flatpak --system remote-ls --runtime flathub 2>/dev/null \
      | grep "^${base}" \
      | awk '{print $2}' \
      | sort -Vr \
      | head -n1)
    ...
    if [ -n "$latest" ]; then
      echo "${base}/x86_64/${latest}"
    else
      echo "⚠️ Could not determine latest branch for $base" >&2
      return 1
    fi
  }

  # Dynamically resolve latest runtimes
  declare -a dynamic_flatpaks=(
    "$(get_latest_runtime_branch runtime/org.freedesktop.Platform.VulkanLayer.MangoHud)"
    "$(get_latest_runtime_branch runtime/org.freedesktop.Platform.VulkanLayer.OBSVkCapture)"
    "$(get_latest_runtime_branch runtime/org.freedesktop.Platform.VulkanLayer.gamescope)"
  )

  # Main Flatpak list (system-level install)
  ...
  for app in "${default_flatpaks[@]}"; do
    fancy " 📦 Installing $app system-wide... please WAIT ..." \
      "sudo flatpak install --system -y flathub $app || echo '⚠️ Failed to install: $app'"
  done
...
Comment=Manage and reinstall Flatpaks (System-wide)
Exec=sudo $HOME/flatpak-manager.sh
...
EOF
  ...
  notify-send "Flatpak Manager" "✅ System-wide setup complete. Launcher created!"
}
