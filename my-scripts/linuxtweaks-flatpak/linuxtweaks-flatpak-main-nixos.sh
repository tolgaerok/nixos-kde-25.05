...
if ! command -v yad &>/dev/null; then
  ...
  if command -v dnf5 &>/dev/null; then
    sudo dnf5 install -y yad || {
      zenity --error --text="Failed to install 'yad' with dnf5. Exiting."
      exit 1
    }
    ...
    nix-env -iA nixpkgs.yad || {
      zenity --error --text="Failed to install 'yad' with nix-env. Exiting."
      exit 1
    }

  else
    zenity --error --text="Unsupported distro or package manager. Please install 'yad' manually."
    exit 1
  fi
fi
...
echo "✅ Icon downloaded to $icon_path" && sleep 0.2 && clear
...
'
  ...
  for app in "${default_flatpaks[@]}"; do
    # Install system-wide
    fancy " 📦 Installing $app..., please WAIT ..." "flatpak install -y --noninteractive --system --or-update flathub \"$app\" || echo '⚠️ Failed to install: $app'"
  done

  # Theme override for rhythm box
  sudo flatpak override --reset org.gnome.Rhythmbox3
  flatpak override --user --reset org.gnome.Rhythmbox3
  sudo flatpak override org.gnome.Rhythmbox3 --env=GTK_THEME=Arc-Dark --filesystem=host
...
#-----------------------------------------#
main_menu() {
  yad --center --width=600 --height=450 --title="LinuxTweaks Flatpak Manager" --image="$icon_path" --text="$prompt_text_2" --list --print-column=1 --column="Option" --column="Description" \
